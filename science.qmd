...
<div class="science-page">
  ...
  <div class="science-page-header">
    ...
    <p class="research-context-note">Ongoing PhD research &middot; Public reference structures (PDB: 5UIW, 7O7F)</p>
  </div>

  <!-- NGL Viewer -->
  <div class="science-viewer-wrap">
    <div id="science-viewer"></div>
    <div class="viewer-loading" id="viewer-loading"><div class="viewer-spinner"></div>Loading structure&hellip;</div>
  ...
  <div class="science-chapter active" id="chapter-0">
    ...
    <h2 class="chapter-title">CCR5: a receptor on the surface of immune cells</h2>
  ...
  <div class="science-chapter" id="chapter-1">
    ...
    <h2 class="chapter-title">A few amino-acid changes. Opposite effects.</h2>
    <div class="chapter-body">
      <p>
      ...
      <div class="nterm-table-wrap">
        ...
        <div class="nterm-table-scroll">
          <table class="nterm-table">
            <thead>
              <tr>
                <th class="row-label">Molecule</th>
                <th class="same">1</th><th class="same">2</th>
                <th class="same">3</th><th class="same">4</th>
                <th class="diff">5</th><th class="diff">6</th>
                <th class="diff">7</th><th class="diff">8</th>
                <th class="diff">9</th><th class="diff">10</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td class="row-label antagonist-label">Blocker</td>
                <td class="same">pGlu</td><td class="same">Gly</td>
                <td class="same">Pro</td><td class="same">Pro</td>
                <td class="diff">Leu</td><td class="diff">Met</td>
                <td class="diff">Ala</td><td class="diff">Leu</td>
                <td class="diff">Gln</td><td class="diff">Ser</td>
              </tr>
              <tr>
                <td class="row-label agonist-label">Activator</td>
                <td class="same">pGlu</td><td class="same">Gly</td>
                <td class="same">Pro</td><td class="same">Pro</td>
                <td class="diff">Gly</td><td class="diff">Asp</td>
                <td class="diff">Ile</td><td class="diff">Val</td>
                <td class="diff">Leu</td><td class="diff">Ala</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  <div class="science-chapter" id="chapter-2">
    ...
    <h2 class="chapter-title">The same receptor. The same binding site. Very different outcomes.</h2>
    <div class="chapter-body">
      <p>
  ...
  <div class="science-chapter" id="chapter-3">
    ...
    <h2 class="chapter-title">From structure to dynamics.</h2>
    <div class="chapter-body">
      <p>
...
<div class="enh-section">
  ...
  <h2>Sampling rare transitions</h2>
  <p>
    The conformational changes described above happen too slowly to observe in a standard simulation. The methods below accelerate this process.
  </p>
  <p>
    At physiological temperature, a molecular dynamics (MD) trajectory may remain confined to a single
    conformational basin over accessible simulation timescales, missing slow transitions such as GPCR activation.
    The underlying philosophy is shared with importance sampling in statistics: instead of drawing samples
    directly from the distribution of interest, we sample from a modified distribution that is easier to
    explore, and then recover the target distribution through reweighting or selection rules.
  </p>
  <p>
    In this example, adaptive biasing is illustrated with metadynamics, which builds a history-dependent bias along
    a chosen collective variable (CV), a low-dimensional descriptor of the protein’s configuration. The bias
    discourages revisiting the same region and encourages transitions. Because the bias is known, free-energy
    profiles along the chosen CV can be estimated afterward (illustrative). The &Delta;G slider below sketches how a
    ligand or mutation shifts that balance.
  </p>
      ...
      <div style="display:flex;align-items:flex-start;gap:20px;margin-top:14px;flex-wrap:wrap;">
        ...
        <div style="flex:1;min-width:160px;">
          ...
          <div style="margin-top:12px;">
            <div class="enh-dg-label">&#916;G (active &minus; inactive)</div>
            <div class="enh-dg-num" id="dGNum">0.0</div>
            <div style="font-size:0.74rem;color:var(--color-text-muted);font-family:var(--font-heading);">kcal/mol &middot; illustrative</div>
          </div>
        </div>
      </div>
  ...
  <p>
    Temperature replica exchange, also known as parallel tempering, follows a different strategy. Multiple copies
    of the system are simulated simultaneously at a series of temperatures. Replicas at higher temperatures cross
    energy barriers more easily and explore a broader range of conformations. At regular intervals, neighbouring
    replicas attempt to exchange configurations according to a Metropolis criterion that depends on their potential
    energies and temperatures.
  </p>
  <p>
    In this way, configurations discovered at high temperature can gradually move down the temperature ladder.
    Exchanges are accepted probabilistically, favouring configurations whose energies are compatible with lower
    temperatures. As a result, the 300&thinsp;K replica samples configurations that would be difficult to reach
    through direct simulation alone while remaining consistent with the target temperature.
  </p>
  ...
  <div class="interactive-zone" style="margin:1rem 0 0;">
    ...
    <div class="iz-body">
      ...
      <div style="font-size:0.74rem;color:var(--color-text-muted);line-height:1.5;margin-top:8px;max-width:640px;">
        Illustrative. The 300&thinsp;K replica (&#9733;) is the simulation with the temperature of interest.
        Temperature replica exchange improves sampling by helping the system overcome energy barriers that would
        be crossed only rarely at 300&thinsp;K. Higher-temperature replicas explore conformations more easily,
        and periodic exchanges allow these configurations to be tested at lower temperatures. Exchanges are
        accepted probabilistically based on energy and temperature, so the 300&thinsp;K replica ultimately
